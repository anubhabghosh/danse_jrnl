...
if __name__ == "__main__":
    ...
    if mode == 'low' or mode == 'high':
        ...
        bias = None
        p = None
        evaluation_mode = 'full_opt_N_bias_{}_p_{}_SMNR_{}_MC3'.format(None, None, smnr_dB_test)

    os.makedirs('./figs/LorenzModel/{}'.format(evaluation_mode), exist_ok=True)

    N_arr = np.array([10, 20, 40, 80, 125, 250, 500, 1000])
    ...
    for N_training in N_arr:
        model_file_saved_dict["{}".format(N_training)] = glob.glob("./models/*LorenzSSM_danse_opt_*_n_3_T_100_N_{}_sigmae2_{}dB_smnr_{}dB*/*best*".format(N_training, sigma_e2_dB_test, smnr_dB_test))[-1]
        model_file_saved_dict_knet["{}".format(N_training)] = glob.glob("./models/*LorenzSSM_KNetUoffline_*_n_3_T_100_N_{}_sigmae2_{}dB_smnr_{}dB*/*best*".format(N_training, sigma_e2_dB_test, smnr_dB_test))[-1]

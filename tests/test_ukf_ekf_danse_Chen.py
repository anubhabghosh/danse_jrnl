...
from config.parameters_opt import get_parameters, A_fn, h_fn, f_lorenz_chen, f_lorenz_chen_ukf, delta_t_chen, J_test
...
def f_lorenz_danse_knet(x, device='cpu'):
    ...
    for j in range(1,J+1):
        F_add = (torch.matrix_power(A*delta_t_chen, j)/math.factorial(j))
        F = torch.add(F, F_add)
...
def test_lorenz(device='cpu', model_file_saved=None, model_file_saved_knet=None, test_data_file=None, test_logfile=None, evaluation_mode='Full', p=0.5, bias=30):
    ...
    if not os.path.isfile(test_data_file):
        ...
        m, n, T_test, N_test, sigma_e2_dB_test, smnr_dB_test = parse("test_trajectories_m_{:d}_n_{:d}_ChenSSM_data_T_{:d}_N_{:d}_sigmae2_{:f}dB_SMNR_{:f}dB.pkl", test_data_file.split('/')[-1])
        ...
        lorenz_model = LorenzSSM(n_states=m, n_obs=n, J=J, delta=delta,
                            alpha=1.0, delta_d=delta_d, decimate=decimate,
    ...
    print("sigma_e2: {}dB, smnr: {}dB, delta_t: {}".format(sigma_e2_dB_test, smnr_dB_test, delta_t_chen), file=orig_stdout)

    print("Fed to Model-based filters: ")
    print("sigma_e2: {}dB, smnr: {}dB, delta_t: {}".format(sigma_e2_dB_test, smnr_dB_test, delta_t_chen))
    ...
    ekf_model = EKF(
        ...
        J=J_test,
        f=f_lorenz_chen,#lorenz_model.A_fn, f_lorenz for KalmanNet paper, f_lorenz_danse for our work
        h=lorenz_model.h_fn,
    ...
    time_elapsed_ukf =  timer() - start_time_ukf
    time_elapsed_ekf = timer() - start_time_ukf
    # Initialize the DANSE model in PyTorch
    danse_model = DANSE(
        n_states=lorenz_model.n_states,
        n_obs=lorenz_model.n_obs,
    ...
    nmse_ekf = nmse_loss(X[:,0:,:], X_estimated_ekf[:,0:,:])
    nmse_ekf_std = nmse_loss_std(X[:,0:,:], X_estimated_ekf[:,0:,:])
    nmse_ukf = nmse_loss(X[:,0:,:], X_estimated_ukf[:,0:,:])
    nmse_ukf_std = nmse_loss_std(X[:,0:,:], X_estimated_ukf[:,0:,:])
    ...
    mse_dB_ekf = mse_loss_dB(X[:,0:,:], X_estimated_ekf[:,0:,:])
    mse_dB_ekf_std = mse_loss_dB_std(X[:,0:,:], X_estimated_ekf[:,0:,:])
    mse_dB_ukf = mse_loss_dB(X[:,0:,:], X_estimated_ukf[:,0:,:])
    mse_dB_ukf_std = mse_loss_dB_std(X[:,0:,:], X_estimated_ukf[:,0:,:])
    ...
    plot_state_trajectory(X=torch.squeeze(X[j,0:,:],0).numpy(), 
                        ...
                        savefig=False,
                        savefig_name="./figs/ChenModel/{}/3dPlot_sigmae2_{}dB_smnr_{}dB_knet.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
    
    plot_3d_state_trajectory(X=torch.squeeze(X[j, 0:, :], 0).numpy(), legend='$\\mathbf{x}^{true}$', m='b-', savefig_name="./figs/ChenModel/{}/chen_x_true_sigmae2_{}dB_smnr_{}dB.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test), savefig=False)
    plot_3d_state_trajectory(X=torch.squeeze(X_estimated_filtered[j], 0).numpy(), legend='$\\hat{\mathbf{x}}_{DANSE}$', m='k-', savefig_name="./figs/ChenModel/{}/chen_x_danse_sigmae2_{}dB_smnr_{}dB.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test), savefig=False)
    plot_3d_measurment_trajectory(Y=torch.squeeze(Y[j, :, :], 0).numpy(), legend='$\\mathbf{y}^{true}$', m='r-', savefig_name="./figs/ChenModel/{}/chen_y_true_sigmae2_{}dB_smnr_{}dB.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test), savefig=False)
    
    
    plot_state_trajectory_w_lims(X=torch.squeeze(X[j,0:,:],0).numpy(), 
                        ...
                        X_est_UKF=torch.squeeze(X_estimated_ukf[j,0:,:], 0).numpy(), 
                        X_est_UKF_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_ukf[j,0:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()), 
                        X_est_DANSE=torch.squeeze(X_estimated_filtered[j], 0).numpy(), 
                        ...
                        #X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(), 
                        savefig=False,
                        savefig_name="./figs/ChenModel/{}/Trajectories_sigma_e2_{}dB_smnr_{}dB.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
    
    plot_state_trajectory_axes(X=torch.squeeze(X[j,0:,:],0).numpy(), 
                                X_est_EKF=torch.squeeze(X_estimated_ekf[0,0:,:],0).numpy(), 
                                X_est_UKF=torch.squeeze(X_estimated_ukf[j,0:,:],0).numpy(), 
                                X_est_DANSE=torch.squeeze(X_estimated_filtered[j],0).numpy(), 
                                #X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(),
                                savefig=False,
                                savefig_name="./figs/ChenModel/{}/AxesWisePlot_sigmae2_{}dB_smnr_{}dB_knet.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
...
if __name__ == "__main__":
    ...
    os.makedirs('./figs/ChenModel/{}'.format(evaluation_mode), exist_ok=True)
    ...
    for smnr_dB in smnr_dB_arr:
        model_file_saved_dict["{}dB".format(smnr_dB)] = glob.glob("./models/*ChenSSM_danse_opt_*n_3*sigmae2_{}dB_smnr_{}dB*/*best*".format(sigma_e2_dB_nominal, smnr_dB))[-1]
        model_file_saved_dict_knet["{}dB".format(smnr_dB)] = None #glob.glob("./models/*Chen*KNetUoffline_*n_3*sigmae2_{}dB_smnr_{}dB*/*best*".format(sigma_e2_dB_test, smnr_dB))[-1]

    test_data_file_dict = {}

    for smnr_dB in smnr_dB_arr:
        test_data_file_dict["{}dB".format(smnr_dB)] = "./data/synthetic_data/test_trajectories_m_3_n_3_ChenSSM_data_T_{}_N_{}_sigmae2_{}dB_smnr_{}dB.pkl".format(T_test, N_test, sigma_e2_dB_test, smnr_dB)
    
    print(model_file_saved_dict)

    test_logfile = "./log/Chen_test_{}_T_{}_N_{}_w_knet.log".format(evaluation_mode, T_test, N_test)
    test_jsonfile = "./log/Chen_test_{}_T_{}_N_{}_w_knet.json".format(evaluation_mode, T_test, N_test)

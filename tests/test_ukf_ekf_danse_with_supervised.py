...
from src.danse_supervised import DANSE_Supervised, push_model
...
def test_dmm_causal_lorenz(dmm_causal_model, saved_model_file, Y, device='cpu'):
...
def test_danse_lorenz_supervised(danse_supervised_model, saved_model_file, Y, device='cpu'):

    danse_supervised_model.load_state_dict(torch.load(saved_model_file, map_location=device))
    ...
    with torch.no_grad():
        ...
        Y_test_batch = Variable(Y, requires_grad=False).type(torch.FloatTensor).to(device)
        X_estimated_pred, Pk_estimated_pred, X_estimated_filtered, Pk_estimated_filtered = danse_supervised_model.compute_predictions(Y_test_batch)
...
def test_lorenz(device='cpu', model_file_saved_danse=None, model_file_saved_danse_supervised=None, 
        model_file_saved_knet=None, model_file_saved_dmm=None, test_data_file=None, test_logfile=None, 
        evaluation_mode='Full', p=0.5, bias=30, dirname=None):
    ...
    #_, inference_mode_dmm, rnn_type_dmm, m, n, T, _, sigma_e2_dB, smnr_dB = parse("{}_dmm_{}_opt_{}_m_{:d}_n_{:d}_T_{:d}_N_{:d}_sigmae2_{:f}dB_SMNR_{:f}dB_lmindel", model_file_saved_dmm.split('/')[-2])
    ...
    # Initialize the DANSE Supervised model in PyTorch
    ...
    print("DANSE Supervised Model file: {}".format(model_file_saved_danse_supervised))

    X_estimated_pred_supervised = None
    X_estimated_filtered_supervised = None
    Pk_estimated_filtered_supervised = None

    start_time_danse_supervised = timer()
    X_estimated_pred_supervised, Pk_estimated_pred_supervised, X_estimated_filtered_supervised, Pk_estimated_filtered_supervised = test_danse_lorenz_supervised(danse_supervised_model=danse_supervised_model, 
                                                                                                                                    saved_model_file=model_file_saved_danse_supervised,
                                                                                                                                    Y=Y,
                                                                                                                                    device=device)
    ...
    dmm_causal_model = DMM(
        obs_dim=lorenz_model.n_obs,
        latent_dim=lorenz_model.n_states,
    ...
    X_estimated_filtered_dmm_causal, Pk_estimated_filtered_dmm_causal = test_dmm_causal_lorenz(dmm_causal_model=dmm_causal_model, 
    ...
    nmse_danse_supervised = nmse_loss(X[:,:,:], X_estimated_filtered_supervised[:,0:,:])
    nmse_danse_supervised_std = nmse_loss_std(X[:,:,:], X_estimated_filtered_supervised[:,0:,:])
    nmse_danse_supervised_pred = nmse_loss(X[:,:,:], X_estimated_pred_supervised[:,0:,:])
    nmse_danse_supervised_pred_std = nmse_loss_std(X[:,:,:], X_estimated_pred_supervised[:,0:,:])
    ...
    mse_dB_danse_supervised = mse_loss_dB(X[:,:,:], X_estimated_filtered_supervised[:,0:,:])
    mse_dB_danse_supervised_std = mse_loss_dB_std(X[:,:,:], X_estimated_filtered_supervised[:,0:,:])
    mse_dB_danse_supervised_pred = mse_loss_dB(X[:,:,:], X_estimated_pred_supervised[:,0:,:])
    mse_dB_danse_supervised_pred_std = mse_loss_dB_std(X[:,:,:], X_estimated_pred_supervised[:,0:,:])
    ...
    print("DANSE Supervised - MSE LOSS:",mse_dB_danse_supervised, "[dB]")
    print("DANSE Supervised - MSE STD:", mse_dB_danse_supervised_std, "[dB]")
    ...
    print("danse sup. (pred.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_danse_supervised_pred, nmse_danse_supervised_pred_std, mse_dB_danse_supervised_pred, mse_dB_danse_supervised_pred_std, time_elapsed_danse_supervised))
    print("danse sup. (fil.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_danse_supervised, nmse_danse_supervised_std, mse_dB_danse_supervised, mse_dB_danse_supervised_std, time_elapsed_danse_supervised))
    ...
    print("danse sup. (pred.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_danse_supervised_pred, nmse_danse_supervised_pred_std, mse_dB_danse_supervised_pred, mse_dB_danse_supervised_pred_std, time_elapsed_danse_supervised), file=orig_stdout)
    print("danse sup. (fil.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_danse_supervised, nmse_danse_supervised_std, mse_dB_danse_supervised, mse_dB_danse_supervised_std, time_elapsed_danse_supervised), file=orig_stdout)
    ...
    plot_state_trajectory(X=torch.squeeze(X[0,1:,:],0).numpy(), 
                        ...
                        X_est_DANSE_sup=torch.squeeze(X_estimated_filtered_supervised[0],0).numpy(),
                        #X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(),
                        savefig=True,
                        savefig_name="./figs/{}/{}/3dPlot_sigmae2_{}dB_smnr_{}dB_knet.pdf".format(dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
    
    plot_3d_state_trajectory(X=torch.squeeze(X[0, 1:, :], 0).numpy(), legend='$\\mathbf{x}^{true}$', m='b-', savefig_name="./figs/{}/{}/lorenz_x_true_sigmae2_{}dB_smnr_{}dB.pdf".format(dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test), savefig=True)
    plot_3d_state_trajectory(X=torch.squeeze(X_estimated_filtered[0], 0).numpy(), legend='$\\hat{\mathbf{x}}_{DANSE}$', m='k-', savefig_name="./figs/{}/{}/lorenz_x_danse_sigmae2_{}dB_smnr_{}dB.pdf".format(dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test), savefig=True)
    plot_3d_measurment_trajectory(Y=torch.squeeze(Y[0, :, :], 0).numpy(), legend='$\\mathbf{y}^{true}$', m='r-', savefig_name="./figs/{}/{}/lorenz_y_true_sigmae2_{}dB_smnr_{}dB.pdf".format(dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test), savefig=True)
    ...
    plot_state_trajectory_axes(X=torch.squeeze(X[0,1:,:],0).numpy(), 
                                ...
                                #X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(),
                                savefig=True,
                                savefig_name="./figs/{}/{}/AxesWisePlot_sigmae2_{}dB_smnr_{}dB_knet.pdf".format(dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
    
    plot_state_trajectory_axes_all(X=torch.squeeze(X[0,1:,:],0).numpy(), 
                                X_est_EKF=torch.squeeze(X_estimated_ekf[0,1:,:],0).numpy(), 
                                X_est_UKF=torch.squeeze(X_estimated_ukf[0,1:,:],0).numpy(), 
                                X_est_DANSE=torch.squeeze(X_estimated_filtered[0],0).numpy(), 
                                #X_est_DMM=torch.squeeze(X_estimated_filtered_dmm_causal[0], 0),
                                ...
                                #X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(),
                                savefig=True,
                                savefig_name="./figs/{}/{}/AxesWiseAllPlot_sigmae2_{}dB_smnr_{}dB_knet.pdf".format(dirname, evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
    ...
    return nmse_ekf, nmse_ekf_std, nmse_danse, nmse_danse_std, nmse_dmm, nmse_dmm_std, nmse_danse_supervised, nmse_danse_supervised_std, nmse_knet, nmse_knet_std, nmse_ukf, nmse_ukf_std, nmse_ls, nmse_ls_std, \
        mse_dB_ekf, mse_dB_ekf_std, mse_dB_danse, mse_dB_danse_std, mse_dB_dmm, mse_dB_dmm_std, mse_dB_danse_supervised, mse_dB_danse_supervised_std, mse_dB_knet, mse_dB_knet_std, mse_dB_ukf, mse_dB_ukf_std, mse_dB_ls, mse_dB_ls_std, \
        time_elapsed_ekf, time_elapsed_danse, time_elapsed_danse_supervised,  time_elapsed_dmm, time_elapsed_knet, time_elapsed_ukf, smnr_dB_test
...
if __name__ == "__main__":
    ...
    if mode == 'low' or mode == 'high':
        ...
        bias = None
        p = None
        evaluation_mode = 'full_opt_bias_{}_p_{}_w_ModTest_Supervised_Ttrain_{}_Ntrain_{}'.format(None, None, T_train, N_train)

    dirname = "LorenzModel_w_sup_DMM_lmindel"
    ...
    nmse_danse_supervised_arr = np.zeros((len(smnr_dB_arr,)))
    ...
    nmse_danse_supervised_std_arr = np.zeros((len(smnr_dB_arr,)))
    ...
    mse_danse_supervised_dB_arr = np.zeros((len(smnr_dB_arr,)))
    ...
    mse_danse_supervised_dB_std_arr = np.zeros((len(smnr_dB_arr,)))
    ...
    t_danse_supervised_arr = np.zeros((len(smnr_dB_arr,)))
    ...
    model_file_saved_dict_danse_sup = {}
    model_file_saved_dict_dmm = {}
    model_file_saved_dict_knet = {}

    for smnr_dB in smnr_dB_arr:
        
        model_file_saved_dict_dmm["{}dB".format(smnr_dB)] = None #glob.glob("./models/*LorenzSSM_dmm_st-l_opt*T_{}_N_{}_sigmae2_{}dB_smnr_{}dB_lmindel/*best*".format(T_train, N_train, sigma_e2_dB_test, smnr_dB))[-1]
        model_file_saved_dict_danse["{}dB".format(smnr_dB)] = glob.glob("./models/*LorenzSSM_danse_opt_*n_3_T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(T_train, N_train, sigma_e2_dB_test, smnr_dB))[-1]
        model_file_saved_dict_danse_sup["{}dB".format(smnr_dB)] = glob.glob("./models/*LorenzSSM_danse_supervised_opt_*n_3_T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(T_train, N_train, sigma_e2_dB_test, smnr_dB))[-1]
        model_file_saved_dict_knet["{}dB".format(smnr_dB)] = None #glob.glob("./models/*Lorenz*KNetUoffline_*n_3_T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(T_train, N_train, sigma_e2_dB_test, smnr_dB))[-1]
    ...
    print("DANSE -- ")
    print([(k, m) for k, m in model_file_saved_dict_danse.items()])
    print("*"*100)
    print("DANSE (Supervised) -- ")
    print([(k, m) for k, m in model_file_saved_dict_danse_sup.items()])
    print("*"*100)
    print("KalmanNet (Unsupervised.) -- ")
    print([(k, m) for k, m in model_file_saved_dict_knet.items()])
    print("*"*100)
    print("DMM (ST-L) (Unsupervised) -- ")
    print([(k, m) for k, m in model_file_saved_dict_dmm.items()])
    print("*"*100)
    test_logfile = "./log/Lorenz_test_{}_T_{}_N_{}_w_knet_sup_ModTest_Ttrain_{}_Ntrain_{}.log".format(evaluation_mode, T_test, N_test, T_train, N_train)
    test_jsonfile = "./log/Lorenz_test_{}_T_{}_N_{}_w_knet_sup_ModTest_Ttrain_{}_Ntrain_{}.json".format(evaluation_mode, T_test, N_test, T_train, N_train)

    for i, smnr_dB in enumerate(smnr_dB_arr):
        
        model_file_saved_danse_i = model_file_saved_dict_danse['{}dB'.format(smnr_dB)]
        model_file_saved_danse_sup_i = model_file_saved_dict_danse_sup['{}dB'.format(smnr_dB)]
        ...
        nmse_ekf_i, nmse_ekf_i_std, nmse_danse_i, nmse_danse_i_std, nmse_dmm_i, nmse_dmm_std_i, nmse_danse_sup_i, nmse_danse_sup_i_std,  nmse_knet_i, nmse_knet_std_i, nmse_ukf_i, nmse_ukf_i_std, nmse_ls_i, nmse_ls_i_std, \
            mse_dB_ekf_i, mse_dB_ekf_std_i, mse_dB_danse_i, mse_dB_danse_std_i, mse_dB_dmm_i, mse_dB_dmm_std_i, mse_dB_danse_sup_i, mse_dB_danse_sup_std_i, mse_dB_knet_i, mse_dB_knet_std_i, mse_dB_ukf_i, mse_dB_ukf_std_i, mse_dB_ls_i, mse_dB_ls_std_i, \
            time_elapsed_ekf_i, time_elapsed_danse_i, time_elapsed_dmm_i, time_elapsed_danse_sup_i, time_elapsed_knet_i, time_elapsed_ukf_i, smnr_dB_i = test_lorenz(device=device, 
            model_file_saved_danse=model_file_saved_danse_i, model_file_saved_dmm=model_file_saved_dmm_i, model_file_saved_danse_supervised=model_file_saved_danse_sup_i, model_file_saved_knet=model_file_saved_knet_i, \
            test_data_file=test_data_file_i, test_logfile=test_logfile, evaluation_mode=evaluation_mode, bias=bias, p=p, dirname=dirname)
        ...
        nmse_danse_supervised_arr[i] = nmse_danse_sup_i.numpy().item()
        ...
        #nmse_danse_supervised_std_arr[i] = nmse_danse_sup_i_std.numpy().item()
        ...
        mse_danse_supervised_dB_arr[i] = mse_dB_danse_sup_i.numpy().item()
        ...
        mse_danse_supervised_dB_std_arr[i] = mse_dB_danse_sup_std_i.numpy().item()
        ...
        t_danse_supervised_arr[i] = time_elapsed_danse_sup_i
    ...
    test_stats['DANSE_sup_mean_nmse'] = nmse_danse_supervised_arr
    ...
    test_stats['DANSE_sup_std_nmse'] = nmse_danse_supervised_std_arr
    ...
    test_stats['DANSE_sup_mean_mse'] = mse_danse_supervised_dB_arr
    ...
    test_stats['DANSE_sup_std_mse'] = mse_danse_supervised_dB_std_arr
    ...
    test_stats['DANSE_sup_time'] = t_danse_supervised_arr
    ...
    plt.errorbar(smnr_dB_arr, nmse_danse_supervised_arr, fmt='c^-', yerr=nmse_danse_std_arr, linewidth=2.0, label="DANSE-Supervised")
    ...
    plt.tight_layout()
    #plt.subplot(212)
    tikzplotlib.save('./figs/{}/{}/NMSE_vs_SMNR_Lorenz.tex'.format(dirname, evaluation_mode))
    plt.savefig('./figs/{}/{}/NMSE_vs_SMNR_Lorenz.pdf'.format(dirname, evaluation_mode))
    ...
    #plt.plot(smnr_dB_arr, t_danse_supervised_arr, 'c^-', linewidth=2.0, label="DANSE-Supervised")
    ...
    plt.legend()
    plt.tight_layout()
    tikzplotlib.save('./figs/{}/{}/InferTime_vs_SMNR_Lorenz_w_knet.tex'.format(dirname, evaluation_mode))
    plt.savefig('./figs/{}/{}/InferTime_vs_SMNR_Lorenz_w_knet.pdf'.format(dirname, evaluation_mode))
    ...
    #plt.errorbar(smnr_dB_arr, mse_danse_supervised_dB_arr, fmt='c^-', yerr=mse_danse_dB_std_arr, linewidth=2.0, label="DANSE-Supervised")
    ...
    plt.tight_layout()
    #plt.subplot(212)
    tikzplotlib.save('./figs/{}/{}/MSE_vs_SMNR_Lorenz_w_knet.tex'.format(dirname, evaluation_mode))
    plt.savefig('./figs/{}/{}/MSE_vs_SMNR_Lorenz_w_knet.pdf'.format(dirname, evaluation_mode))

...
def test_lorenz(device='cpu', model_file_saved=None, model_file_saved_knet=None, test_data_file=None, 
                test_logfile=None, evaluation_mode='Full', p=0.5, bias=30, figdir='LorenzSSMn2'):

    ssm_type, rnn_type, m, n, T, _, sigma_e2_dB, smnr_dB = parse("{}_danse_opt_{}_m_{:d}_n_{:d}_T_{:d}_N_{:d}_sigmae2_{:f}dB_SMNR_{:f}dB", model_file_saved.split('/')[-2])
    ...
    H_tensor = torch.from_numpy(jacobian(lorenz_model.h_fn, torch.randn(lorenz_model.n_states,)).numpy()).type(torch.FloatTensor)
    ...
    plot_state_trajectory(X=torch.squeeze(X[0,0:,:],0).numpy(), 
                        ...
                        savefig=False,
                        savefig_name="./figs/{}/{}/3dPlot_sigmae2_{}dB_smnr_{}dB_knet.pdf".format(figdir, evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
    ...
    plot_state_trajectory_axes(X=torch.squeeze(X[0,0:,:],0).numpy(), 
                                ...
                                X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(),
                                savefig=True,
                                savefig_name="./figs/{}/{}/AxesWisePlot_sigmae2_{}dB_smnr_{}dB_knet.pdf".format(figdir, evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
    
    plot_state_trajectory_axes_all(X=torch.squeeze(X[0,0:,:],0).numpy(), 
                                X_est_EKF=torch.squeeze(X_estimated_ekf[0,0:,:],0).numpy(), 
                                X_est_UKF=torch.squeeze(X_estimated_ukf[0,0:,:],0).numpy(), 
                                X_est_DANSE=torch.squeeze(X_estimated_filtered[0],0).numpy(), 
                                X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(),
                                savefig=True,
                                savefig_name="./figs/{}/{}/AxesWiseSubPlot_sigmae2_{}dB_smnr_{}dB_knet.pdf".format(figdir, evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
...
if __name__ == "__main__":
    ...
    if mode == 'low' or mode == 'high':
        ...
        bias = None
        p = None
        evaluation_mode = 'full_opt_bias_{}_p_{}_w_RN{}_Ttrain_{}_Ntrain_{}'.format(None, None, n_obs, T_train, N_train)

    #dirname = 'LorenzSSMn{}'.format(n_obs) # The directory name is the same as the SSM type
    dirname = 'LorenzSSMrn{}'.format(n_obs) # The directory name is the same as the SSM type
    ...
    for smnr_dB in smnr_dB_arr:
        model_file_saved_dict["{}dB".format(smnr_dB)] = glob.glob("./models/*LorenzSSMrn{}_danse_opt_*n_{}_T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(n_obs, n_obs, T_train, N_train, sigma_e2_dB_test, smnr_dB))[-1]
        model_file_saved_dict_knet["{}dB".format(smnr_dB)] = glob.glob("./models/*LorenzSSMrn{}*KNetUoffline_*n_{}_T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(n_obs, n_obs, T_train, N_train, sigma_e2_dB_test, smnr_dB))[-1]

    print(model_file_saved_dict_knet)
    test_data_file_dict = {}

    for smnr_dB in smnr_dB_arr:
        test_data_file_dict["{}dB".format(smnr_dB)] = "./data/synthetic_data/test_trajectories_m_3_n_{}_{}_data_T_{}_N_{}_sigmae2_{}dB_smnr_{}dB.pkl".format(n_obs, dirname, T_test, N_test, sigma_e2_dB_test, smnr_dB)
        
    test_logfile = "./log/LorenzSSMrn{}_test_{}_T_{}_N_{}_w_knet_Ttrain_{}_Ntrain_{}.log".format(n_obs, evaluation_mode, T_test, N_test, T_train, N_train)
    test_jsonfile = "./log/LorenzSSMrn{}_test_{}_T_{}_N_{}_w_knet_Ttrain_{}_Ntrain_{}.json".format(n_obs, evaluation_mode, T_test, N_test, T_train, N_train)
    ...
    for i, smnr_dB in enumerate(smnr_dB_arr):
        ...
        nmse_ekf_i, nmse_ekf_i_std, nmse_danse_i, nmse_danse_i_std, nmse_knet_i, nmse_knet_std_i, nmse_ukf_i, nmse_ukf_i_std, nmse_ls_i, nmse_ls_i_std, \
            ...
            evaluation_mode=evaluation_mode, bias=bias, p=p, figdir=dirname)

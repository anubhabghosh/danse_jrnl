...
def test_lorenz(device='cpu', model_file_saved=None, model_file_saved_mismatched=None,  model_file_saved_knet=None, model_file_saved_knet_mismatched=None, test_data_file=None, test_logfile=None, evaluation_mode='Full', p=0.5, bias=30):
    ...
    print("Testing DANSE Mismatched ...", file=orig_stdout)
    # Initialize the mismatched DANSE model in PyTorch
    danse_model_mismatched = DANSE(
        n_states=lorenz_model.n_states,
        n_obs=lorenz_model.n_obs,
        mu_w=lorenz_model.mu_w,
        C_w=lorenz_model.Cw,
    ...
    X_estimated_mismatched_pred = None
    X_estimated_mismatched_filtered = None
    Pk_estimated_mismatched_filtered = None

    start_time_danse_mismatched = timer()
    X_estimated_mismatched_pred, Pk_estimated_mismatched_pred, X_estimated_mismatched_filtered, Pk_estimated_mismatched_filtered = test_danse_lorenz(danse_model=danse_model_mismatched, 
                                                                                                                                    saved_model_file=model_file_saved_mismatched,
                                                                                                                                    Y=Y,
                                                                                                                                    device=device)
    time_elapsed_danse_mismatched = timer() - start_time_danse_mismatched
    ...
    print("Testing KalmanNet Mismatched ...", file=orig_stdout)
    # Initialize the KalmanNet model in PyTorch
    knet_model_mismatched = KalmanNetNN(
        n_states=lorenz_model.n_states,
        n_obs=lorenz_model.n_obs,
        n_layers=1,
        device=device
    ...
    knet_model_mismatched.Build(f=fn, h=hn)
    knet_model_mismatched.ssModel = lorenz_model

    start_time_knet_mismatched = timer()

    X_estimated_filtered_mismatched_knet = test_knet_lorenz(knet_model=knet_model_mismatched, 
                                                saved_model_file=model_file_saved_knet_mismatched,
                                                Y=Y,
                                                device=device)
    
    time_elapsed_knet_mismatched = timer() - start_time_knet_mismatched
    ...
    nmse_danse_mismatched = nmse_loss(X[:,:,:], X_estimated_mismatched_filtered[:,0:,:])
    nmse_danse_mismatched_std = nmse_loss_std(X[:,:,:], X_estimated_mismatched_filtered[:,0:,:])
    ...
    nmse_knet_mismatched = nmse_loss(X[:,:,:], X_estimated_filtered_mismatched_knet[:,0:,:])
    nmse_knet_mismatched_std = nmse_loss_std(X[:,:,:], X_estimated_filtered_mismatched_knet[:,0:,:])
    ...
    mse_dB_danse_mismatched = mse_loss_dB(X[:,:,:], X_estimated_mismatched_filtered[:,0:,:])
    mse_dB_danse_mismatched_std = mse_loss_dB_std(X[:,:,:], X_estimated_mismatched_filtered[:,0:,:])
    ...
    mse_dB_knet_mismatched = mse_loss_dB(X[:,:,:], X_estimated_filtered_mismatched_knet[:,0:,:])
    mse_dB_knet_mismatched_std = mse_loss_dB_std(X[:,:,:], X_estimated_filtered_mismatched_knet[:,0:,:])
    ...
    print("danse - mismatched (fil.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_danse_mismatched, nmse_danse_mismatched_std, mse_dB_danse_mismatched, mse_dB_danse_mismatched_std, time_elapsed_danse_mismatched))
    ...
    print("knet - mismatched (fil.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_knet_mismatched, nmse_knet_mismatched_std, mse_dB_knet_mismatched, mse_dB_knet_mismatched_std, time_elapsed_knet_mismatched))
    ...
    print("danse - mismatched (fil.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_danse_mismatched, nmse_danse_mismatched_std, mse_dB_danse_mismatched, mse_dB_danse_mismatched_std, time_elapsed_danse_mismatched), file=orig_stdout)
    ...
    print("knet - mismatched (fil.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_knet_mismatched, nmse_knet_mismatched_std, mse_dB_knet_mismatched, mse_dB_knet_mismatched_std, time_elapsed_knet_mismatched), file=orig_stdout)
    ...
    plot_3d_state_trajectory(X=torch.squeeze(X[0, :, :], 0).numpy(), legend='$\\mathbf{x}^{true}$', m='b-', savefig_name="./figs/LorenzModel/{}/lorenz_x_true_sigmae2_{}dB_smnr_{}dB.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test), savefig=True)
    ...
    plot_state_trajectory_w_lims(X=torch.squeeze(X[0,:,:],0).numpy(), 
                        ...
                        X_est_UKF=torch.squeeze(X_estimated_ukf[0,:,:], 0).numpy(), 
                        X_est_UKF_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_ukf[0,:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()), 
                        X_est_DANSE=torch.squeeze(X_estimated_filtered[0], 0).numpy(), 
    ...
    return nmse_ekf, nmse_ekf_std, nmse_danse, nmse_danse_std, nmse_danse_mismatched, nmse_danse_mismatched_std, nmse_knet, nmse_knet_std, nmse_knet_mismatched, nmse_knet_mismatched_std, nmse_ukf, nmse_ukf_std, nmse_ls, nmse_ls_std, \
        mse_dB_ekf, mse_dB_ekf_std, mse_dB_danse, mse_dB_danse_std, mse_dB_danse_mismatched, mse_dB_danse_mismatched_std, mse_dB_knet, mse_dB_knet_std, mse_dB_knet_mismatched, mse_dB_knet_mismatched_std, mse_dB_ukf, mse_dB_ukf_std, mse_dB_ls, mse_dB_ls_std, \
...
if __name__ == "__main__":
    ...
    smnr_dB_nominal = 20.0 # Choice of SMNR for trained model
    device = 'cpu'
    ...
    if mode == 'low' or mode == 'high':
        ...
        bias = None
        p = None
        evaluation_mode = 'full_opt_bias_{}_p_{}_diff_sigmaw_at_smnr_{}_T_{}_Ttrain_{}_Ntrain_{}'.format(None, None, smnr_dB_nominal, T_test, T_train, N_train)
    ...
    nmse_danse_mismatched_arr = np.zeros((len(smnr_dB_arr,)))
    nmse_knet_arr = np.zeros((len(smnr_dB_arr,)))
    nmse_knet_mismatched_arr = np.zeros((len(smnr_dB_arr,)))
    ...
    nmse_danse_mismatched_std_arr = np.zeros((len(smnr_dB_arr,)))
    nmse_knet_std_arr = np.zeros((len(smnr_dB_arr,)))
    nmse_knet_mismatched_std_arr = np.zeros((len(smnr_dB_arr,)))
    ...
    mse_danse_mismatched_dB_arr = np.zeros((len(smnr_dB_arr,)))
    mse_knet_dB_arr = np.zeros((len(smnr_dB_arr,)))
    mse_knet_mismatched_dB_arr = np.zeros((len(smnr_dB_arr,)))
    ...
    mse_danse_mismatched_dB_std_arr = np.zeros((len(smnr_dB_arr,)))
    mse_knet_dB_std_arr = np.zeros((len(smnr_dB_arr,)))
    mse_knet_mismatched_dB_std_arr = np.zeros((len(smnr_dB_arr,)))
    ...
    model_file_saved_dict_danse_mismatched = {}
    model_file_saved_dict_knet_mismatched = {}
    ...
    for smnr_dB in smnr_dB_arr:
        ...
        model_file_saved_dict_danse_mismatched["{}dB".format(smnr_dB)] = glob.glob("./models/*LorenzSSM_danse_opt_*n_3_T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(T_train, N_train, sigma_e2_dB_test, smnr_dB_nominal))[-1]
        ...
        model_file_saved_dict_knet_mismatched["{}dB".format(smnr_dB)] = glob.glob("./models/*Lorenz*KNetUoffline_*n_3_T_{}_N_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(T_train, N_train, sigma_e2_dB_test, smnr_dB_nominal))[-1]
    ...
    test_logfile = "./log/Lorenz_test_{}_T_{}_N_{}_w_knet_diff_sigmaw_at_smnr_{}_Ttrain_{}_Ntrain_{}.log".format(evaluation_mode, T_test, N_test, smnr_dB_nominal, T_train, N_train)
    test_jsonfile = "./log/Lorenz_test_{}_T_{}_N_{}_w_knet_diff_sigmaw_at_smnr_{}_Ttrain_{}_Ntrain_{}.json".format(evaluation_mode, T_test, N_test, smnr_dB_nominal, T_train, N_train)

    for i, smnr_dB in enumerate(smnr_dB_arr):
        
        model_file_saved_danse_i = model_file_saved_dict_danse['{}dB'.format(smnr_dB)]
        model_file_saved_danse_mismatched_i = model_file_saved_dict_danse_mismatched['{}dB'.format(smnr_dB)]
        ...
        model_file_saved_knet_mismatched_i = model_file_saved_dict_knet_mismatched['{}dB'.format(smnr_dB)]

        nmse_ekf_i, nmse_ekf_i_std, nmse_danse_i, nmse_danse_i_std, nmse_danse_mismatched_i, nmse_danse_mismatched_i_std, nmse_knet_i, \
        nmse_knet_std_i, nmse_knet_mismatched_i, nmse_knet_mismatched_i_std, nmse_ukf_i, nmse_ukf_i_std, nmse_ls_i, nmse_ls_i_std, \
        mse_dB_ekf_i, mse_dB_ekf_std_i, mse_dB_danse_i, mse_dB_danse_std_i, mse_dB_danse_mismatched_i, mse_dB_danse_mismatched_std_i, \
        mse_dB_knet_i, mse_dB_knet_std_i,  mse_dB_knet_mismatched_i, mse_dB_knet_mismatched_std_i, mse_dB_ukf_i, mse_dB_ukf_std_i, mse_dB_ls_i, mse_dB_ls_std_i, \
        time_elapsed_ekf_i, time_elapsed_danse_i, time_elapsed_knet_i, time_elapsed_ukf_i, smnr_dB_i = test_lorenz(device=device, 
        model_file_saved=model_file_saved_danse_i, model_file_saved_knet=model_file_saved_knet_i, 
        model_file_saved_mismatched=model_file_saved_danse_mismatched_i, model_file_saved_knet_mismatched=model_file_saved_knet_mismatched_i, 
        test_data_file=test_data_file_i, test_logfile=test_logfile, 
        ...
        nmse_danse_mismatched_arr[i] = nmse_danse_mismatched_i.numpy().item()
        nmse_knet_arr[i] = nmse_knet_i.numpy().item()
        nmse_knet_mismatched_arr[i] = nmse_knet_mismatched_i.numpy().item()
        ...
        nmse_danse_mismatched_std_arr[i] = nmse_danse_mismatched_i_std.numpy().item()
        nmse_knet_mismatched_std_arr[i] = nmse_knet_mismatched_i_std.numpy().item()
        ...
        mse_danse_dB_arr[i] = mse_dB_danse_mismatched_i.numpy().item()
        mse_knet_dB_arr[i] = mse_dB_knet_mismatched_i.numpy().item()
        ...
        mse_danse_mismatched_dB_std_arr[i] = mse_dB_danse_mismatched_std_i.numpy().item()
        mse_knet_mismatched_dB_std_arr[i] = mse_dB_knet_mismatched_std_i.numpy().item()
    ...
    test_stats['DANSE_mis_mean_nmse'] = nmse_danse_mismatched_arr
    test_stats['KNET_mean_nmse'] = nmse_knet_arr
    test_stats['KNET_mis_mean_nmse'] = nmse_knet_mismatched_arr
    ...
    test_stats['DANSE_mis_std_nmse'] = nmse_danse_mismatched_std_arr
    test_stats['KNET_mis_std_nmse'] = nmse_knet_mismatched_std_arr
    ...
    test_stats['DANSE_mis_mean_mse'] = mse_danse_mismatched_dB_arr
    test_stats['KNET_mis_mean_mse'] = mse_knet_mismatched_dB_arr
    ...
    test_stats['DANSE_mis_std_mse'] = mse_danse_mismatched_dB_std_arr
    test_stats['KNET_mis_std_mse'] = mse_knet_mismatched_dB_std_arr
    ...
    plt.errorbar(smnr_dB_arr, nmse_danse_mismatched_arr, fmt='b*-.', yerr=nmse_danse_mismatched_std_arr, linewidth=2.0, label="DANSE-Mismatched")
    plt.errorbar(smnr_dB_arr, nmse_knet_mismatched_arr, fmt='ys-.', yerr=nmse_knet_mismatched_std_arr,  linewidth=1.0, label="KalmanNet-Mismatched")
    ...
    plt.errorbar(smnr_dB_arr, mse_danse_mismatched_dB_arr, fmt='b*-.', yerr=mse_danse_mismatched_dB_std_arr, linewidth=2.0, label="DANSE-Mismatched")
    plt.errorbar(smnr_dB_arr, mse_knet_mismatched_dB_arr, fmt='ys-.', yerr=mse_knet_mismatched_dB_std_arr,  linewidth=1.0, label="KalmanNet-Mismatched")
    plt.xlabel('SMNR (in dB)')

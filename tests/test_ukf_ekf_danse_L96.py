...
from config.parameters_opt import get_parameters, A_fn, h_fn, delta_t_L96
...
def test_lorenz(device='cpu', model_file_saved=None, model_file_saved_knet=None, test_data_file=None, test_logfile=None, evaluation_mode='Full', p=0.5, bias=30):
    ...
    plot_state_trajectory_axes(X=torch.squeeze(X[j,1:,:],0).numpy(), 
                                ...
                                #X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(),
                                savefig=True,
                                savefig_name="./figs/Lorenz96Model/{}/AxesWisePlot_sigmae2_{}dB_smnr_{}dB_knet.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test))
...
if __name__ == "__main__":
    ...
    for smnr_dB in smnr_dB_arr:
        model_file_saved_dict["{}dB".format(smnr_dB)] = glob.glob("./models/*Lorenz96SSM_danse_opt_*n_{}*sigmae2_{}dB_smnr_{}dB*/*best*".format(n_obs, sigma_e2_dB_nominal, smnr_dB))[-1]
    ...
    for smnr_dB in smnr_dB_arr:
        test_data_file_dict["{}dB".format(smnr_dB)] = "./data/synthetic_data/test_trajectories_m_{}_n_{}_Lorenz96SSM_data_T_{}_N_{}_sigmae2_{}dB_smnr_{}dB.pkl".format(n_states, n_obs, T_test, N_test, sigma_e2_dB_test, smnr_dB)

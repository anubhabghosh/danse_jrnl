...
def test_ukf_lorenz_one_step(X, Y, ukf_model):

    X_estimated_ukf_pred, Pk_estimated_ukf_pred, mse_arr_uk_lin_pred, mse_arr_ukf_pred, \
        X_estimated_ukf_filtered, Pk_estimated_ukf_filtered, mse_arr_uk_lin_filtered, mse_arr_ukf_filtered = ukf_model.run_mb_filter(X, Y)
    
    return X_estimated_ukf_pred, Pk_estimated_ukf_pred, mse_arr_uk_lin_pred, mse_arr_ukf_pred, \
        X_estimated_ukf_filtered, Pk_estimated_ukf_filtered, mse_arr_uk_lin_filtered, mse_arr_ukf_filtered
...
def test_lorenz(device='cpu', model_file_saved=None, model_file_saved_knet=None, test_data_file=None, test_logfile=None, evaluation_mode='Full', p=0.5, bias=30):
    ...
    # Get the estimates using an extended Kalman filter model
    X_estimated_ukf_pred = None
    Pk_estimated_ukf_pred = None
    X_estimated_ukf_filtered = None
    Pk_estimated_ukf_filtered = None
    start_time_ukf = timer()
    X_estimated_ukf_pred, Pk_estimated_ukf_pred, mse_arr_uk_lin_pred, mse_arr_ukf_pred, \
        X_estimated_ukf_filtered, Pk_estimated_ukf_filtered, mse_arr_uk_lin_filtered, mse_arr_ukf_filtered = test_ukf_lorenz_one_step(X=X, Y=Y, ukf_model=ukf_model)
    time_elapsed_ukf = timer() - start_time_ukf
    
    Y_estimated_ukf_pred = torch.einsum('ij,nkj->nki', torch.from_numpy(lorenz_model.H).type(torch.FloatTensor), X_estimated_ukf_pred)
    Py_estimated_ukf_pred = torch.einsum('nkij,lj->nkil',
        torch.einsum('ij,nkjj->nkij', torch.from_numpy(lorenz_model.H).type(torch.FloatTensor), Pk_estimated_ukf_pred), torch.from_numpy(lorenz_model.H).type(torch.FloatTensor)) + torch.from_numpy(lorenz_model.Cw).type(torch.FloatTensor)
    ...
    X_estimated_pred, Pk_estimated_pred, X_estimated_filtered, Pk_estimated_filtered = test_danse_lorenz(danse_model=danse_model, 
                                                                                                ...
                                                                                                Y=Y,
                                                                                                device=device)
    Y_estimated_pred = torch.einsum('ij,nkj->nki', torch.from_numpy(lorenz_model.H).type(torch.FloatTensor), X_estimated_pred)
    Py_estimated_pred = torch.einsum('nkij,lj->nkil',
        torch.einsum('ij,nkjj->nkij', torch.from_numpy(lorenz_model.H).type(torch.FloatTensor), Pk_estimated_pred), torch.from_numpy(lorenz_model.H).type(torch.FloatTensor)) + torch.from_numpy(lorenz_model.Cw).type(torch.FloatTensor)
    ...
    nmse_ukf = nmse_loss(X[:,0:,:], X_estimated_ukf_filtered[:,0:,:])
    nmse_ukf_std = nmse_loss_std(X[:,0:,:], X_estimated_ukf_filtered[:,0:,:])
    nmse_ukf_pred = nmse_loss(X[:,0:,:], X_estimated_ukf_pred[:,0:,:])
    nmse_ukf_pred_std = nmse_loss_std(X[:,0:,:], X_estimated_ukf_pred[:,0:,:])
    ...
    mse_dB_ukf = mse_loss_dB(X[:,0:,:], X_estimated_ukf_filtered[:,0:,:])
    mse_dB_ukf_std = mse_loss_dB_std(X[:,0:,:], X_estimated_ukf_filtered[:,0:,:])
    mse_dB_ukf_pred = mse_loss_dB(X[:,0:,:], X_estimated_ukf_pred[:,0:,:])
    mse_dB_ukf_pred_std = mse_loss_dB_std(X[:,0:,:], X_estimated_ukf_pred[:,0:,:])
    ...
    print("ukf (pred.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_ukf_pred, nmse_ukf_pred_std, mse_dB_ukf_pred, mse_dB_ukf_pred_std, time_elapsed_ukf))
    print("ukf (fil.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_ukf, nmse_ukf_std, mse_dB_ukf, mse_dB_ukf_std, time_elapsed_ukf))
    ...
    print("ukf (pred.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {:.4f} secs".format(N_test, nmse_ukf_pred, nmse_ukf_pred_std, mse_dB_ukf_pred, mse_dB_ukf_pred_std, time_elapsed_ukf), file=orig_stdout)
    print("ukf (fil.), batch size: {}, nmse: {:.4f} ± {:.4f}[dB], mse: {:.4f} ± {:.4f}[dB], time: {} secs".format(N_test, nmse_ukf, nmse_ukf_std, mse_dB_ukf, mse_dB_ukf_std, time_elapsed_ukf), file=orig_stdout)
    ...
    #plot_state_trajectory(X=torch.squeeze(X[0,0:,:],0).numpy(), 
    #                    X_est_EKF=torch.squeeze(X_estimated_ekf[0,0:,:],0).numpy(), 
    #                    X_est_UKF=torch.squeeze(X_estimated_ukf[0,0:,:],0).numpy(), 
    ...
    plot_3d_state_trajectory(X=torch.squeeze(X[0, 0:, :], 0).numpy(), legend='$\\mathbf{x}^{true}$', m='b-', savefig_name="./figs/LorenzModel/{}/lorenz_x_true_sigmae2_{}dB_smnr_{}dB.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test), savefig=True)
    plot_3d_state_trajectory(X=torch.squeeze(X_estimated_pred[0], 0).numpy(), legend='$\\hat{\mathbf{x}}_{DANSE}$', m='k-', savefig_name="./figs/LorenzModel/{}/lorenz_x_danse_sigmae2_{}dB_smnr_{}dB.pdf".format(evaluation_mode, sigma_e2_dB_test, smnr_dB_test), savefig=True)
    ...
    plot_state_trajectory_w_lims(X=torch.squeeze(X[0,0:,:],0).numpy(), 
                            #X_est_KF=torch.squeeze(X_estimated_kf[0,0:,:], 0).numpy(), 
                            #X_est_KF_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_kf[0,0:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()), 
                            X_est_UKF=torch.squeeze(X_estimated_ukf_pred[0,0:,:], 0).numpy(), 
                            X_est_UKF_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_ukf_pred[0,0:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()), 
                            ...
                            #X_est_DANSE_sup_std=np.diag(torch.squeeze(Pk_estimated_filtered_sup[0,0:,:], 0).numpy()).sqrt(), 
                            #X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(), 
                            savefig=True,
    ...
    plot_meas_trajectory_w_lims(Y=torch.squeeze(Y[0,:,:],0).numpy(), 
                            #X_est_KF=torch.squeeze(X_estimated_kf[0,0:,:], 0).numpy(), 
                            #X_est_KF_std=np.sqrt(torch.diagonal(torch.squeeze(Pk_estimated_kf[0,0:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()), 
                            Y_pred_UKF=torch.squeeze(Y_estimated_ukf_pred[0,0:,:], 0).numpy(), 
                            Y_pred_UKF_std=np.sqrt(torch.diagonal(torch.squeeze(Py_estimated_ukf_pred[0,0:,:,:], 0), offset=0, dim1=1,dim2=2).numpy()), 
    ...
    #plot_state_trajectory_axes(X=torch.squeeze(X[0,0:,:],0).numpy(), 
    #                            #X_est_EKF=torch.squeeze(X_estimated_ekf[0,0:,:],0).numpy(), 
    #                            X_est_UKF=torch.squeeze(X_estimated_ukf_pred[0,0:,:],0).numpy(), 
    #                            X_est_DANSE=torch.squeeze(X_estimated_pred[0],0).numpy(), 
    #                            #X_est_KNET=torch.squeeze(X_estimated_filtered_knet[0], 0).numpy(),

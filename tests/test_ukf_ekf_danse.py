...
if __name__ == "__main__":
    ...
    if mode == 'low' or mode == 'high':
        evaluation_mode = 'partial_{}_bias_{}_p_{}'.format(mode, bias, p)
    else:
        bias = None
        p = None
        evaluation_mode = 'full_bias_{}_p_{}'.format(None, None)
    ...
    for smnr_dB in smnr_dB_arr:
        model_file_saved_dict["{}dB".format(smnr_dB)] = glob.glob("./models/*Lorenz*danse*sigmae2_{}dB_smnr_{}dB*/*best*".format(sigma_e2_dB_test, smnr_dB))[-1]
    ...
    plt.plot(smnr_dB_arr, nmse_ekf_arr, 'rd--', linewidth=1.5, label="EKF")
    plt.plot(smnr_dB_arr, nmse_danse_arr, 'bo-', linewidth=2.0, label="DANSE")
    plt.plot(smnr_dB_arr, nmse_ukf_arr, 'ks-', linewidth=2.0, label="UKF")
    ...
    plt.plot(smnr_dB_arr, mse_ekf_dB_arr, 'rd--', linewidth=1.5, label="EKF")
    plt.plot(smnr_dB_arr, mse_ukf_dB_arr, 'ks-', linewidth=1.5, label="UKF")
